...
class TestHuffmanCoding:
    def test_number_of_unique_chars(self):
        ...
        result = l_huffman_coding.unique_chars(code)
        assert len(result) == 6
    ...
    def test_huffman(self):
        code = "A_DEAD_DAD_CEDED_A_BAD_BABE_A_BEADED_ABACA_BED"
        result = l_huffman_coding.encode_header(code)
        result += l_huffman_coding.encode(code)
        decoded_value = l_huffman_coding.decode(result)
        assert (
